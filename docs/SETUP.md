# Getting Started with bot_army_outreach

This guide walks you through setting up Outreach Bot for local development.
...
DATABASE_NAME=bot_army_outreach_dev
```
...
Outreach Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_outreach/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_outreach/skills/example.ex lib/bot_army_outreach/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyOutreach.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_outreach.command.classify"]
...
```elixir
# In lib/bot_army_outreach/application.ex or similar

defmodule BotArmyOutreach.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyOutreach.Skills.Classify,
      BotArmyOutreach.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_outreach/skills/my_skill.ex
...
This creates a release named `outreach_bot` that is deployed to `/opt/ergon/releases/outreach_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/outreach_bot/`
