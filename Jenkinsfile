pipeline {
  ...
  environment {
    BOT_NAME = 'outreach_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-outreach"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/outreach_bot/bin/outreach_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyOutreach.Release.create()'"
          $RELEASE_BIN eval 'BotArmyOutreach.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyOutreach.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyOutreach.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_outreach/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_outreach/releases/start_erl.data)
        fi
